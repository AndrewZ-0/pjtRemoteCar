from raspiSioServer import RaspiSioServer
...
class gpioSioDriver:
    ...
    def start(self):
        ...
        while True:
            if len(self.rss.commandsStack) > 0:
                cmdPacket = self.rss.commandsStack[0]
                cmd, direction = self.unpackCmd(cmdPacket)

                self.executeCommand(cmd, direction)
                
                del self.rss.commandsStack[0]
            sleep(0.1)
        GPIO.cleanup()
...
if __name__ == "__main__":
    ...
    rs_thread = Thread(target = rss.start, daemon = True)
    rs_thread.start()
    
    gd = gpioSioDriver(rss)
